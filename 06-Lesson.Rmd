...
After today you will know some additional ways to manipulate data frames. 

## Grammar for data

The **dplyr** package has functions ('verbs') that perform common data operations on data frames. Filtering for rows, selecting specific columns, re-ordering rows, adding new columns, and summarizing data. Let's review using the `airquality` data frame.
```{r}
library(dplyr)
head(airquality)
dim(airquality)
...
These properties make it easy to chain together many simple steps to get at something that is more complex.
