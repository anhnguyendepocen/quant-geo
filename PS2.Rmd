...
**1** The following values are the annual number of hurricanes that have hit the United States since 1990. Answer the questions by typing R commands.
...
**6** The dataset *hflights* from the **hflights** package contains all 227,496 flights that departed Houston in 2011. Using the functions in the **dplyr** package
...
**7** Answer the following questions using the tornado data set (http://myweb.fsu.edu/jelsner/temp/data/Tornadoes.txt).

a. Use the functions `group_by()` and `summarize()` to determine the number of tornadoes by each state. (5)

b. Create a new column with path length in meters (the length variable is in miles). Create a new data frame by removing rows with EF damage rating below 3. Group by year and summarize the average path length. Make a graph with year on the x axis and average path length on the y axis. (10)
