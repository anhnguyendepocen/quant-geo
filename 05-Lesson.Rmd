...
Today we learn how to work with data frames.

??Problem Set 2 is posted on Canvas. It's due next Tuesday (September 18).
...
We saw last week that the mean is a statistic that provides a model for our data. Typically there are more data values near the mean than far from it.

The median is a statistic that is defined as the "middle" value.  Given a set of data values arranged from lowest value to highest value, the median is the middle value.

For example, consider a set of seven data values. Here generated randomly but exactly reproducible.
```{r}
set.seed(3043)
data <- rnorm(7)
sort(data)
